...
protected:
	...
	read_num(m_Tp *ret, std::string *errtext) ATTRIBUTE_NONNULL((2));

	/// Write nonnegative number t to fp (undefined behaviour if t < 0)
	template<typename m_Tp> bool
	write_num(m_Tp t, std::string *errtext);
...
template<typename m_Tp> bool
Database::read_num(m_Tp *ret, std::string *errtext)
{
	int ch(getch());
	if(likely(ch != EOF)) {
...
template<typename m_Tp> bool
Database::write_num(m_Tp t, std::string *errtext)
{
