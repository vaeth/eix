...
{
	vector<string> v = split_string(eixrc[s], spaces, true, true, true);
	if(v.size() & 1)
	{
...
run_update_eix(int argc, char *argv[])
{
	...
	add_pathnames(excluded_list, split_string(eixrc["EXCLUDE_OVERLAY"], spaces, true, true, true), true);
	vector<Pathname> add_list;
	add_pathnames(add_list, split_string(eixrc["ADD_OVERLAY"], spaces, true, true, true), true);
